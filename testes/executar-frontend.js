...

{
  ...
  RAMAApi.lerCapas = (ids) => {
    capas.pedidos.push(ids.slice());
    return Promise.resolve({ ok: true, dados: { [ids[0]]: { imagem: "capa-" + ids[0], versao: "v1" } } });
  };
  recarregarModulo();
  guardado.clear();
  imagens.pedidos.length = 0;
  const aCapa = await RAMAImagens.capa("c1", "v1");
  t.igual("a capa da campanha vem pela ação dela", aCapa, "capa-c1");
  t.iguais("  e não pela das fotos", imagens.pedidos, []);
  await RAMAImagens.capa("c1", "v1");
  t.igual("  e não é pedida de novo", capas.pedidos.length, 1);

  /* Pedir primeiro o que está à vista é otimização; a imagem chegar é
     requisito. Há navegador e há situação (aba em segundo plano, página
     que ainda não foi pintada) em que o aviso de "está à vista" não vem
     nunca — e sem a rede de segurança a imagem não apareceria jamais.
     Foi assim que a capa da campanha não aparecia na lista. */
  ...
  imagens.resposta = (ids) => ({
    ok: true,
    dados: ids.reduce((mapa, id) => { mapa[id] = { imagem: "tardia-" + id, versao: "v1" }; return mapa; }, {}),
  });

  const caixa = {
    dataset: {}, textContent: "AB", filho: null,
    appendChild(n) { this.filho = n; },
  };
  RAMAImagens.aplicar(caixa, { tipo: "foto", id: "p77", versao: "v1" });
  await new Promise((r) => setTimeout(r, 1500));
  t.ok("a imagem chega mesmo se o navegador nunca avisar que o cartão está à vista",
    !!caixa.filho && caixa.filho.src === "tardia-p77");
